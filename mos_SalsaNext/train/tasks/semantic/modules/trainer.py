...
class Trainer():
    ...
    def __init__(self, ARCH, DATA, datadir, logdir, path=None, uncertainty=False):
        ...
        self.info = {"train_update": 0, "train_loss": 0, "train_acc": 0, "train_iou": 0, "valid_loss": 0, "valid_acc": 0, "valid_iou": 0, "best_train_iou": 0, "best_val_iou": 0}
        ...
        self.optimizer = optim.SGD([{'params': self.model.parameters()}], lr=self.ARCH["train"]["lr"], momentum=self.ARCH["train"]["momentum"], weight_decay=self.ARCH["train"]["w_decay"])
        ...
        if self.path is not None:
            ...
            w_dict = torch.load(path + "/SalsaNext", map_location=lambda storage, loc: storage)
    ...
    def train(self):
        ...
        for epoch in range(self.epoch, self.ARCH["train"]["max_epochs"]):
            ...
            state = {'epoch': epoch, 'state_dict': self.model.state_dict(), 'optimizer': self.optimizer.state_dict(), 'info': self.info, 'scheduler': self.scheduler.state_dict()}
            ...
            if self.uncertainty:
                ...
                if i % self.ARCH["train"]["report_batch"] == 0:
                    print('Lr: {lr:.3e} | '
                          ...
                          'IoU {iou.val:.3f} ({iou.avg:.3f}) | [{estim}]'.format(epoch,
                                                                                 i,
                                                                                 len(train_loader),
        ...
        with torch.no_grad():
            ...
            if self.uncertainty:
                print('Validation set:\n'
                      ...
                      'Acc avg {acc.avg:.3f}\n'
                      'IoU avg {iou.avg:.3f}'.format(batch_time=self.batch_time_e, loss=losses, jac=jaccs, wces=wces, hetero=hetero_l, acc=acc, iou=iou))
                ...
                print('Validation set:\n'
                      ...
                      'WCE avg {wces.avg:.4f}\n'
                      'Acc avg {acc.avg:.3f}\n'
                      'IoU avg {iou.avg:.3f}'.format(batch_time=self.batch_time_e, loss=losses, jac=jaccs, wces=wces, acc=acc, iou=iou))
                ...
                # print also classwise
                for i, jacc in enumerate(class_jaccard):
                    print('IoU class {i:} [{class_str:}] = {jacc:.5f}'.format(i=i, class_str=class_func(i), jacc=jacc))
                    save_to_log(self.log, 'log.txt', 'IoU class {i:} [{class_str:}] = {jacc:.5f}'.format(i=i, class_str=class_func(i), jacc=jacc))
                    self.info["valid_classes/" + class_func(i)] = jacc
